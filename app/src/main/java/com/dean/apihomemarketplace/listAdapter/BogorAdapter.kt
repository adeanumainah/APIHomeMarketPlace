...
class BogorAdapter(private val listener: (Bogor) -> Unit)
    : RecyclerView.Adapter<BogorAdapter.ViewHolder>() {

    private val listBgr = ArrayList<Bogor>()

    fun setData(items: ArrayList<Bogor>){
        listBgr.clear()
        listBgr.addAll(items)
        //mensyncron data
        notifyDataSetChanged()
    }

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): BogorAdapter.ViewHolder {
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        holder.bind(listBgr[position], listener)
    }

    override fun getItemCount(): Int = listBgr.size

    class ViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        fun bind(bgr: Bogor, listener: (Bogor) -> Unit) {
            with(itemView) {
...
//                Log.d("Cek DataDi Detail",urlImg)
                Glide.with(itemView)
                    .load("urlImg")
                    .placeholder(R.drawable.house3)
                    .centerCrop()
                    .into(img_list_staggered)

                tv_name_stglist.setText(bgr.nameBgr)
                tv_address_stglist.setText(bgr.addressBgr)
...
//                context.startActivity(page)
                ...
                itemView.setOnClickListener { listener(bgr) }
            }
        }
